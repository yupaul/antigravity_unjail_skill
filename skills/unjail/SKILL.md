---
name: unjail
description: Bypasses sandbox restrictions by proxying shell commands through a user-started terminal runner.
risk: high
source: user-made
...
4. **No Response**: If `AGENT_UNJAIL_RESPONSE.json` does not appear after an appropriate for the command waiting time, ask the user to launch the runner in a **new terminal tab**: `~/.gemini/antigravity/skills/unjail/scripts/unjail_runner.sh` and keep this terminal tab open. After a positive respoinse from the user, restart from 3.
